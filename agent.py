...
Ask ONE question only. Mix up the phrasing each time."""
            ...
            user_prompt = f"""User asks: {user_text}

If it's NOT about movies, politely say: "סורי, אני כאן בשביל להמליץ על סרטים! 🎬 רוצה עוד המלצה?" (in Hebrew) or "Sorry, I'm here to help with movie recommendations! Want another one?" (in English)

If it IS about movies, answer naturally."""
...
def chat():
    try:
        ...
        if session.get("done"):
            reply = call_openai_safe(user_text, "post_recommendation", answers, [], language)
            if not reply:
                reply = "סורי, אני כאן בשביל סרטים! רוצה המלצה נוספת?" if language == "Hebrew" else "Sorry, I'm here for movies! Want another recommendation?"
